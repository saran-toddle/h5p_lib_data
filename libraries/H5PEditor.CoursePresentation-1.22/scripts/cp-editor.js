...
H5PEditor.CoursePresentation = function (parent, field, params, setValue) {
    ...
    H5P.DragNBar.FormManager.call(
        ...
        parent,
        {
            doneButtonLabel: H5PEditor.t(
                'H5PEditor.CoursePresentation',
                'done'
            ),
            deleteButtonLabel: H5PEditor.t(
                'H5PEditor.CoursePresentation',
                'remove'
            ),
...
H5PEditor.CoursePresentation.prototype.initializeDNB = function () {
    ...
    var action = H5PEditor.CoursePresentation.findField(
        'action',
        elementFields
    );
        ...
        function (libraries) {
            ...
            for (var i = 0; i < libraries.length; i++) {
                if (libraries[i].restricted !== true) {
                    ...
                    const libraryId = libraries[i].name
                        .split('.')[1]
                        .toLowerCase();
                    if (dropdownMenus[libraryId] === undefined) {
                        ...
                        buttons.push(
                            that.addDNBButtonGroup(
                                libraries[i],
                                dropdownMenus[libraryId]
                            )
                        );
                    }
                }
            ...
            that.dnb = new H5P.DragNBar(
                buttons,
                that.cp.$current,
                that.$editor,
                { $blurHandlers: that.cp.$boxWrapper, libraries: libraries }
            );
            ...
            that.dnb.dnr.on('stoppedResizing', function () {
                ...
                elementParams.width =
                    that.dnb.$element.width() /
                    (that.cp.$current.innerWidth() / 100);
                elementParams.height =
                    that.dnb.$element.height() /
                    (that.cp.$current.innerHeight() / 100);
                elementParams.y =
            ...
            that.dnb.dnd.releaseCallback = function () {
                ...
                var element =
                    that.elements[that.cp.$current.index()][
                        that.dnb.$element.index()
                    ];
                ...
                if (that.dnb.newElement) {
                    that.cp.$boxWrapper
                        .add(
                            that.cp.$boxWrapper.find(
                                '.h5p-presentation-wrapper:first'
                            )
                        )
                        .css('overflow', '');
                    ...
                    if (
                        ...
                        H5P.libraryFromString(params.action.library)
                            .machineName === 'H5P.ContinuousText'
                    ) {
                        H5P.ContinuousText.Engine.run(that);
                        if (!that.params.ct) {
            ...
            that.dnb.on('paste', function (event) {
                ...
                if (pasted.from === H5PEditor.CoursePresentation.clipboardKey) {
                    ...
                    if (!pasted.generic) {
                        ...
                        that.dnb.focus(
                            that.addElement(pasted.specific, options)
                        );
                        ...
                        if (
                            pasted.generic.library.split(' ')[0] ===
                        ...
                        that.showConfirmationDialog({
                            ...
                            dialogText: H5PEditor.t(
                                'H5P.DragNBar',
                                'unableToPaste'
                            ),
                            confirmText: H5PEditor.t(
                                'H5PEditor.CoursePresentation',
                                'ok'
                            )
                        });
                    ...
                    if (that.supported(pasted.generic.library)) {
                        ...
                        that.dnb.focus(
                            that.addElement(pasted.generic.library, options)
                        );
                    } else {
                        that.showConfirmationDialog({
                            headerText: H5PEditor.t('core', 'pasteError'),
    ...
    for (var i = 0; i < this.elements.length; i++) {
        ...
        for (var j = 0; j < this.elements[i].length; j++) {
            ...
            for (var k = 0; k < this.elements[i][j].children.length; k++) {
                if (
                    this.elements[i][j].children[k].validate() === false &&
                    valid
                ) {
                    valid = false;
                }
...
H5PEditor.CoursePresentation.prototype.updateNavigationLine = function (index) {
    ...
    this.cp.slides.forEach(function (instanceArray, slideNumber) {
        ...
        if (
            ...
            that.cp.elementInstances[slideNumber].forEach(function (
                elementInstance
            ) {
...
H5PEditor.CoursePresentation.prototype.animateNavigationLine = function (
    ...
    var $selectedNext =
        direction == 1
            ? $selectedProgressPart.prev()
            : $selectedProgressPart.next();
...
H5PEditor.CoursePresentation.prototype.updateKeyword = function (
    ...
    H5PEditor.$('[role="menuitem"].h5p-current .h5p-keyword-title').text(
        keyword
    );
};
...
H5PEditor.CoursePresentation.prototype.generateForm = function (
    ...
    element.$form.find('.field-name-displayAsButton').each(function () {
        ...
        var buttonSizeField = ns
            .$(this)
            .parent()
            .find('.field-name-buttonSize');
...
H5PEditor.CoursePresentation.prototype.hideFields = function (
    ...
    for (var i = 0; i < fields.length; i++) {
        var field = H5PEditor.CoursePresentation.findField(
            fields[i],
            elementFields
        );
        if (field) {
    ...
    dnbElement.contextMenu.on('contextMenuRemove', function () {
        ...
        confirmationDialog.on('confirmed', () => {
            ...
            self.removeElement(
                ...
                element.$wrapper,
                elementParams.action !== undefined &&
                    H5P.libraryFromString(elementParams.action.library)
                        .machineName === 'H5P.ContinuousText'
            );
            self.dnb.blurAll();
        });
...
H5PEditor.CoursePresentation.prototype.redrawElement = function (
    ...
    repeat =
        typeof repeat === 'undefined'
            ? elements.length - 1 - elementIndex
            : repeat;
    ...
    $wrapper.remove();
    var instance =
        this.cp.children[slideIndex].addChild(elementParams).instance;
