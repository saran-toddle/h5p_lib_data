...
H5P.Blanks = (function ($, Question) {
    ...
    Blanks.prototype.registerDomElements = function () {
            ...
            if (type === 'H5P.Image') {
                if (media.params.file) {
                    ...
                    self.setImage(media.params.file.path, {
                        disableImageZooming:
                            self.params.media.disableImageZooming || false,
        ...
        self.setIntroduction(
            '<div id="' + labelId + '">' + self.params.text + '</div>'
        );

        // Register task content area
        self.setContent(self.createQuestions(labelId), {
            class: self.params.behaviour.separateLines
                ? 'h5p-separate-lines'
                : ''
        });
    ...
    Blanks.prototype.registerButtons = function () {
        ...
        const notShowCheckAnsButton = !!(
            this.contentData && this.contentData.previousState !== undefined
        );
    ...
    Blanks.prototype.handleBlanks = function (question, handler) {
        ...
        while (clozeStart !== -1 && clozeEnd !== -1) {
            ...
            question =
                question.slice(0, clozeStart - 1) +
                replacer +
                question.slice(clozeEnd);
    ...
    Blanks.prototype.getxAPIDefinition = function () {
        ...
        for (var i = 0; i < this.params.questions.length; i++) {
            var question = this.handleBlanks(
                this.params.questions[i],
                function (solution) {
                    ...
                    crp += (!crp ? '' : '[,]') + solution.solutions.join('/');
        ...
        definition.correctResponsesPattern = [crp];
        definition.caseSensitive = this.params.behaviour.caseSensitive;
...
H5P.Blanks.parseText = function (question) {
    ...
    return tokenizeQuestionText(replaceHtmlTags(question, '')).map(function (
        part
    ) {
